...
| **M3** | [Parse SQL](#m3--parse-sql) | lexer, parser, AST | 4 / 10 | ◐ |
...
| M3.2 | Expression parser: MySQL precedence, operators, literals, `?` placeholders | parser | — | M3.1 | ◐ | precedence matches a real server across a generated expression corpus — **the parser is done and the corpus is not yet committed**, exactly as M2.21 stood before its first successful run. `tools/capture-precedence.mjs` generates expressions, has a real 8.4 evaluate each, and the replay parses and evaluates the *tree*; the test-side evaluator contains no precedence knowledge at all, so it cannot share the parser's mistake and a misgrouped tree comes out as a wrong number. Integers only, deliberately: `/` returns DECIMAL and the float family is inexact, so including them would test formatting rather than grouping. It runs on the server the `type-vectors` job already has up rather than starting a second one. Two `sql_mode` flags reach the parser and *regroup* rather than re-spell: `HIGH_NOT_PRECEDENCE` (`NOT 2 = 1` is 1 by default and 0 under the mode — most `NOT x = y` pairs agree by accident of boolean algebra, and 2 is the smallest operand where they part) and `PIPES_AS_CONCAT`, which moves `||` from the bottom of the table to just below the unary operators. A bug found while wiring that up: with `PIPES_AS_CONCAT` off, `a \|\| b` indexed one past the end of the precedence table and threw a `TypeError` — a crash from ordinary input, which ground rule 5 forbids |
