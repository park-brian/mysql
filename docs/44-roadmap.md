...
| **M2** | [Types and collations](#m2--types-and-collations) | the part everyone else gets wrong | 23 / 24 | ◐ |
...
| M2.21 | `tools/capture-types.mjs` and the informational `type-vectors` CI job (D-34) | test | [43 §4](./43-testing.md) | M2.11 | ◐ | the tool, the job and the replay harness exist and gate whatever is committed; **the corpus is still empty**, because these vectors can only be produced by a real MySQL 8.4 and the environment this was built in has neither a server nor a Docker daemon. The CI job uploads them as an artifact for a human to commit — the one acceptance clause not yet met. **D-34's SQL is wrong**: it writes `WEIGHT_STRING(s LEVELS 1)`, but the keyword is `LEVEL` (singular) and 8.4 rejects `LEVELS` outright. The tool omits the clause entirely, because bare `WEIGHT_STRING` already returns what that collation's `strnxfrm` produces — naming a level is redundant for an `_ai_ci` collation and wrong for a multi-level one |
...
| Core bundle, gzipped | 48.3 KB (`@myjs/protocol` 40.3 KB, `@myjs/charsets` 16.7 KB, `@myjs/types` 23.5 KB), plus 48.6 KB of UCA weights in a chunk nothing loads until a `_0900_` collation is asked for | < 500 KB |
