...
| **M2** | [Types and collations](#m2--types-and-collations) | the part everyone else gets wrong | 20 / 24 | ◐ |
...
| M2.24 | `StorageValue` and `toDriverValue()` in `@myjs/types` — the half of D-32 no item created. Add `decodeStorageValue(fieldType, bytes)` over the existing per-type decoders | types | [15](./15-wire-types.md), [24](./24-column-encodings.md) | M2.12 | ☐ | D-15's mapping is a named function with a test per row, and `decodeStorageValue` is what doc 28's `custom-data` tag calls — so M2.13 has something to return into |
